# =============================================================
# app.py - STREAMLIT UI (2 TAB - MỨC 2)
# =============================================================
# Tab 1: Chat tài liệu - RAG Pipeline (Mức 1)
# Tab 2: Phân tích đa phương thức - Multimodal (Mức 2)
#         - Ảnh: Visual Reasoning (suy luận logic)
#         - Video/Audio: Gemini File API
#
# Chạy: streamlit run app.py
#
# Token Management:
# - st.cache_resource: Cache RAGChain → không khởi tạo lại mỗi click
# - Hiển thị provider đang dùng (Gemini/Groq/Ollama)
# - File API: upload 1 lần, Google lưu 48h → tiết kiệm băng thông
# =============================================================
...
# =============================================================
# CACHE COMPONENTS (Token Management)
# =============================================================
...
def _render_rag_response(response: dict):
    """Hiển thị response RAG dưới dạng UI đẹp."""
    import re
    ...
    with col1:
        ...
        source = re.sub(r"\*+", "", source).strip().replace("Nguồn:", "").strip()
        st.markdown("📄 **Nguồn:**")
        st.caption(source)
    with col2:
...
def render_tab_multimodal():
    """
    Tab 2: Multimodal - Phân tích ảnh và video/audio.

    Kỹ thuật Mức 2:
    - Visual Reasoning: Suy luận logic từ ảnh (không chỉ nhận diện)
    - Gemini File API: Upload video/audio lên Google server (không dùng base64)
    - Structured Extraction: Trích xuất thông tin ra JSON
    """
    st.markdown("### 📸 Phân tích Đa Phương Thức (Multimodal - Mức 2)")
    st.markdown("*Phân tích ảnh tài liệu SGU hoặc upload video/audio*")

    # Giải thích kỹ thuật Mức 2
...
def _render_image_analysis():
    """Phân tích ảnh với Visual Reasoning."""
...
def _render_image_result(result: dict):
    ...
    # Visual Reasoning — điểm nổi bật của Mức 2
...
def _render_media_analysis():
    ...
    with col1:
        uploaded_media = st.file_uploader(
            ...
            help="Hỗ trợ MP4, MOV, MP3, WAV... Tối đa 100MB (Gemini File API)."
        )
    ...
    with col2:
        if uploaded_media:
            ...
            # Hiển thị giải thích File API
            st.markdown("**Quy trình File API:**")
            st.markdown("""
            ...
            # Cảnh báo file lớn
            if size_mb > 50:
                st.warning(f"⚠️ File lớn ({size_mb:.1f}MB) có thể mất 1-3 phút để xử lý.")
...
def _render_media_result(result: dict):
    """Hiển thị kết quả phân tích video/audio."""
    st.markdown("---")
    st.markdown("### 📋 Kết quả Phân tích")

    content_type = result.get("content_type", "không xác định")
    st.info(f"🎬 **Loại nội dung:** {content_type}")
    ...
    if action_items:
        st.markdown("**✅ Danh sách việc cần làm (To-do list):**")
        for item in action_items:
            if isinstance(item, dict):
                ...
                label = f"- **{task}**"
                if assignee:
                    label += f" — Người thực hiện: {assignee}"
                if deadline:
    ...
    with col1:
        st.markdown(f"{conf_emoji} **Độ tin cậy:** {confidence.upper()}")
    with col2:
        st.markdown('<span class="provider-badge">🤖 Gemini File API</span>',
                    unsafe_allow_html=True)
